...
class NMDA(bp.TwoEndConn):    
    ...
    def update(self, _t):
        self.x += bp.backend.reshape(self.pre.spike, (-1, 1)) \
                  * self.conn_mat        
        self.s, self.x = self.integral(
            self.s, self.x, _t, self.tau_rise, self.tau, self.a
        )
        
        self.g.push(self.g_max * self.s)
        g_inf = 1 + self.cc_Mg \
                    / self.beta * bp.backend.exp(-self.alpha * self.post.V)
        g_inf = 1 / g_inf
...
class AMPA(bp.TwoEndConn):
    ...
    def update(self, _t):
        ...
        self.s += bp.backend.reshape(self.pre.spike, (-1, 1)) \
                  * self.conn_mat
        self.g.push(self.g_max * self.s)
