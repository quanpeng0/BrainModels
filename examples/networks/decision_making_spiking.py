...
f = 0.25  #!! 0.15 # proportion of neurons activated by one of the two stimulus
N_A = int(f * N_E)
N_B = int(f * N_E)
...
time_scale = 10.
pre_period = 1500. / time_scale
stim_period = 1000.
delay_period = 3000. / time_scale
...
for i in range(N_A):
    ...
    #weight_NMDA[i, N_A + N_B: N_E] = w_neg
...
'''for i in range(N_E):
    weight_NMDA[i, i] = 0.  #del self conn'''
...
'''for i in range(N_E):
    weight_AMPA[i, i] = 0.  #del self conn'''
## TODO: here use different weights for AMPA and NMDA!!!
print(f"Check contraints: Weight sum {weight_NMDA.sum(axis=0)[0]} should be equal to N_E = {N_E}")

'''weight_GABAa = np.ones((N_I, N_I), dtype = np.float)
for i in range(N_I):
    weight_GABAa[i, i] = 0.  #del self conn'''

## set background params
poisson_freq = 9800.  #!! 2400.  #Hz
g_max_ext2E_AMPA = 2.1  * 1e-3 * net_scale ##TODO: check if scaled?  #uS
g_max_ext2I_AMPA = 1.62 * 1e-3 * net_scale #uS

g_max_E2E_AMPA  = 0.012 * 1e-3 * net_scale
g_max_E2E_NMDA  = 0.040 * 1e-3 * net_scale
g_max_E2I_AMPA  = 0.015 * 1e-3 * net_scale
g_max_E2I_NMDA  = 0.045 * 1e-3 * net_scale
g_max_I2E_GABAa = 1.60  * 1e-3 * net_scale
g_max_I2I_GABAa = 1.25  * 1e-3 * net_scale
...
NMDA_syn = get_NMDA(mode = 'matrix') # check with NMDA
GABAa_syn = bpmodels.synapses.get_GABAa1(mode = 'matrix')
## TODO: try different mode here

# def neurons
## def E neurons/pyramid neurons
neu_E = bp.NeuGroup(model = LIF_neu, geometry = N_E, monitors = ['spike', 'input', 'V'])
...
syn_E2E_AMPA = bp.SynConn(model = AMPA_syn, 
                          ...
                          delay = delay_syn)
syn_E2E_AMPA.pars['g_max'] = g_max_E2E_AMPA * weight_AMPA  #!! weight_NMDA ## weight normalize
    ...
    def update(ST, _t):
        if _t > t_start and _t < t_end:
            if _t - ST['t_last_change'][0] >= t_interval:
                ...
                ST['t_last_change'] = _t
                ST['spike'] = np.random.random(ST['spike'].shape) < ST['freq'] * dt_s
            else:
                ST['spike'] = np.random.random(ST['spike'].shape) < ST['freq'] * dt_s
        else:
...
possion_neu_2B = get_poisson(t_start = pre_period, 
                             ...
                             mean_freq = mu_B,
                             var_freq = 20.)  #!! 10.
neu_input2A = bp.NeuGroup(model = possion_neu_2A, geometry = N_A, monitors = ['freq'])
neu_input2B = bp.NeuGroup(model = possion_neu_2B, geometry = N_B, monitors = ['freq'])
...
pdb.set_trace()
# visualize
fig, gs = bp.visualize.get_figure(6, 2, 4, 8)

fig.add_subplot(gs[0:2, 0])
bp.visualize.raster_plot(net.ts, neu_E.mon.spike, markersize=1, ylim = [0, N_A])
fig.add_subplot(gs[2:4, 0])
bp.visualize.raster_plot(net.ts, neu_E.mon.spike, markersize=1, ylim = [N_A, N_A +N_B])
fig.add_subplot(gs[0:2, 1])
plt.plot(net.ts, neu_E.mon.spike[:, 0 : N_A].sum(axis = 1))
fig.add_subplot(gs[2:4, 1])
plt.plot(net.ts, neu_E.mon.spike[:, N_A : N_A + N_B].sum(axis = 1))

fig.add_subplot(gs[4, 1])
plt.plot(net.ts, neu_E.mon.V[:, N_A - 3])
plt.plot(net.ts, neu_E.mon.spike[:, N_A - 3])


fig.add_subplot(gs[4, 0])
plt.plot(net.ts, neu_input2A.mon.freq[:, 0])
plt.plot(net.ts, neu_input2B.mon.freq[:, 0])
...
plt.show()'''
# TODO: may we paint two together? A: You shall not pass!